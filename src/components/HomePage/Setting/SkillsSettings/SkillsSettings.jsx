...
export default function SkillsSettings() {
  ...
  const handleAddSkill = async () => {
    ...
    try {
      const response = await fetch('https://glittering-cannoli-bc84ac.netlify.app/api/skills', {
        method: 'POST',
  ...
  const handleUpdateSkill = async () => {
    ...
    try {
      const response = await fetch(`https://glittering-cannoli-bc84ac.netlify.app/api/skills/${formDataSkill.idSkill}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteSkill = async (id) => {
    try {
      const response = await fetch(`https://glittering-cannoli-bc84ac.netlify.app/api/skills/${id}`, {
        method: 'DELETE',
      });
